...
`;
	...
	<table role="${"grid"}"><thead><th scope="${"col"}">Id </th>
			...
			<th><a href="${"/order/new"}">${validate_component(Plus_square, "PlusSquare").$$render($$result, {}, {}, {})}</a></th></thead>
		<tbody>${each(value.resource, ({ OrderId, OrderDate }) => {
        return `<tr><td scope="${"row"}">${escape(OrderId)}</td>
					<td>${escape(OrderDate)}</td>
					<td><a href="${"/order/" + escape(OrderId, true)}">${validate_component(Edit, "Edit").$$render($$result, {}, {}, {})}</a></td>
				</tr>`;
      })}</tbody></table>
`;
    }(__value);
