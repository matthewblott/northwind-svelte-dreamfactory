...
`;
	...
	<table role="${"grid"}"><thead><th scope="${"col"}">Id </th>
			...
			<th><a href="${"/shippers/new"}">${validate_component(Plus_square, "PlusSquare").$$render($$result, {}, {}, {})}</a></th></thead>
		<tbody>${each(value.resource, ({ ShipperId, CompanyName }) => {
        return `<tr><td scope="${"row"}">${escape(ShipperId)}</td>
					<td>${escape(CompanyName)}</td>
					<td><a href="${"/shippers/" + escape(ShipperId, true)}">${validate_component(Edit, "Edit").$$render($$result, {}, {}, {})}</a></td>
				</tr>`;
      })}</tbody></table>
`;
    }(__value);
