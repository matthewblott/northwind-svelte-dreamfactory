...
`;
	...
	<table role="${"grid"}"><thead><th scope="${"col"}">Id </th>
			...
			<th><a href="${"/suppliers/new"}">${validate_component(Plus_square, "PlusSquare").$$render($$result, {}, {}, {})}</a></th></thead>
		<tbody>${each(value.resource, ({ SupplierId, CompanyName }) => {
        return `<tr><td scope="${"row"}">${escape(SupplierId)}</td>
					<td>${escape(CompanyName)}</td>
					<td><a href="${"/suppliers/" + escape(SupplierId, true)}">${validate_component(Edit, "Edit").$$render($$result, {}, {}, {})}</a></td>
				</tr>`;
      })}</tbody></table>
`;
    }(__value);
