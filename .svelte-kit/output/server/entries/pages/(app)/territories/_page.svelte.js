...
`;
	...
	<table role="${"grid"}"><thead><th scope="${"col"}">Id </th>
			...
			<th><a href="${"/territories/new"}">${validate_component(Plus_square, "PlusSquare").$$render($$result, {}, {}, {})}</a></th></thead>
		<tbody>${each(value.resource, ({ TerritoryId, TerritoryDescription }) => {
        return `<tr><td scope="${"row"}">${escape(TerritoryId)}</td>
					<td>${escape(TerritoryDescription)}</td>
					<td><a href="${"/territories/" + escape(TerritoryId, true)}">${validate_component(Edit, "Edit").$$render($$result, {}, {}, {})}</a></td>
				</tr>`;
      })}</tbody></table>
`;
    }(__value);
